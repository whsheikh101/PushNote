...
open class AutoCompleteTextField:UITextField, UITableViewDataSource, UITableViewDelegate{
    ...
    fileprivate func commonInit(){
        ...
        autoCompleteAttributes![NSAttributedString.Key.font] = UIFont(name: "HelveticaNeue-Bold", size: 12)
    ...
    fileprivate func setupAutocompleteTable(_ view:UIView){
        ...
        let tableView = UITableView(frame: CGRect(x: self.frame.origin.x, y: self.frame.origin.y + self.frame.height + 18.0 , width: screenSize.width - (self.frame.origin.x * 2), height: 30.0))
       // tableView.backgroundColor = UIColor(red: 239.0/255.0, green: 239.0/255.0, blue: 244.0/255.0, alpha: 1.0)
    ...
    fileprivate func reload(){
        if enableAttributedText{
            let attrs = [NSAttributedString.Key.foregroundColor:autoCompleteTextColor, NSAttributedString.Key.font:UIFont.systemFont(ofSize: 12.0)] as? [String : Any]
            ...
            if autoCompleteStrings != nil{
                for i in 0..<autoCompleteStrings!.count{
                    ...
                    let attString = NSMutableAttributedString(string: autoCompleteStrings![i],
                                                              attributes: attrs as! [NSAttributedString.Key : AnyObject])
